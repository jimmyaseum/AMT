...
import com.app.amtcust.model.response.RegistrationModel
import com.app.amtcust.model.response.RegistrationResponse
import com.app.amtcust.receiver.OTP_Receiver
...
//        requestsmspermission();
    ...
    override fun initializeView() {
        ...
        RootRef = FirebaseDatabase.getInstance().reference.child(ChatConstant.F_CUSTOMER)
        ...
        if (currentUser == null) {
            mAuth!!.signInWithEmailAndPassword(userModel.EmailID, "123456").addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    ...
                    RootRef!!.child(currentUserID).child("usertype").setValue("customer")
                    RootRef!!.child(currentUserID).child("name").setValue(userModel.Name)
                    ...
                    RootRef!!.child(currentUserID).child("image").setValue(userModel.CustomerImage)
                        .addOnCompleteListener(OnCompleteListener<Void?> { task ->
                            if (task.isSuccessful) {
                                ...
                                startActivity(intent)
                                finishAffinity()
                                Toast.makeText(this@VerifyOtpActivity,"success...", Toast.LENGTH_SHORT).show()
                            }
                        })
                } else {
