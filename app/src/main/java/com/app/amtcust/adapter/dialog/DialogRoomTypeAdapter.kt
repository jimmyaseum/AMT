...
class DialogRoomTypeAdapter(val selectedpos: String, val context: Context?, private val arrData: ArrayList<HotelRoom>) : RecyclerView.Adapter<DialogRoomTypeAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
         holder.bindItems(context!!, position, arrData, recyclerRowClick!!, selectedpos)
    }
    ...
    class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bindItems(
            context: Context,
            position: Int,
            model: ArrayList<HotelRoom>,
            recyclerClickListener: RecyclerClickListener,
            selectedpos: String
        ) {
            this.context = context

            if(selectedpos == model[position].RoomType) {
                model[position].isSelected = true
            }

            if(model[position].RoomType != null) {
                itemView.radioRoomType.text = model[position].RoomType
            }
