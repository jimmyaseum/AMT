...
import com.app.amtcust.adapter.IndiaTourListAdapter
...
import kotlinx.android.synthetic.main.fragment_india_tour.view.*

class IndiaTourFragment : BaseFragment(), RecyclerClickListener {

    private var views: View? = null
    private val repo by lazy { NetworkRepo() }
    lateinit var adapter: IndiaTourListAdapter
    ...
    override fun onCreateView(
        ...
        views = inflater.inflate(R.layout.fragment_india_tour, container, false)
        initializeView()
        return views
    }
    ...
    override fun initializeView() {
        ...
        views!!.rvIndiaTourList.layoutManager = staggeredGridLayoutManager
    ...
    private fun GetSectorList() {
        launchProgress()
        repo.getSectorList("domestic", listners = ResponseListner {
            ...
            if (it.status) {
                if (it.data?.Status == 200) {
                    ...
                    if(arrSectorList.size > 0) {
                        views!!.rvIndiaTourList.adapter = IndiaTourListAdapter(activity,arrSectorList,this)
                    }
