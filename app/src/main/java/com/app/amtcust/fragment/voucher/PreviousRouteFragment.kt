...
class PreviousRouteFragment  : BaseFragment(), RecyclerClickListener {
    ...
    private fun GetPreviousRouteList() {

        showProgress()

        val userId = sharedPreference?.getPreferenceString(PrefConstants.PREF_USER_ID)!!.toInt()
        repo.getFutureRouteList(userId,7, listners = ResponseListner {
            if (it.status) {
