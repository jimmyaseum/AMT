...
public class ChatMessageAdapter extends RecyclerView.Adapter<ChatMessageAdapter.MessageViewHolder>
{
    ...
    public void onBindViewHolder(@NonNull MessageViewHolder holder, @SuppressLint("RecyclerView") final int position) {
        ...
        usersRef = FirebaseDatabase.getInstance().getReference().child(ChatConstant.F_EMPLOYEE).child(fromUserID);
        usersRef.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if(snapshot.hasChild("message")) {
                    String receiverImage = snapshot.child("message").getValue().toString();
                    Picasso.get().load(receiverImage).placeholder(R.drawable.ic_profile).placeholder(R.drawable.loading_icon).into(holder.receiver_profile_image);
                }
            }
            @Override
        ...
        usersRef = FirebaseDatabase.getInstance().getReference().child(ChatConstant.F_CUSTOMER).child(messageSenderId);
        usersRef.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if(snapshot.hasChild("message")) {
                    String senderImage = snapshot.child("message").getValue().toString();
                    Picasso.get().load(senderImage).placeholder(R.drawable.ic_profile).placeholder(R.drawable.loading_icon).into(holder.sender_profile_image);
                }
            }
            @Override
            public void onCancelled(@NonNull DatabaseError error) {
        ...
        else if (fromMessageType.equals("image"))
        {
            if(fromUserID.equals(messageSenderId)) {
                ...
                Picasso.get().load(messages.getMessage()).resize(150,120).placeholder(R.drawable.loading_icon).into(holder.sender_image);
                holder.sender_date_time.setText(messages.getTime());
                ...
                Picasso.get().load(messages.getMessage()).resize(150,120).placeholder(R.drawable.loading_icon).into(holder.receiver_image);
                holder.receiver_date_time.setText(messages.getTime());
                holder.itemView.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View view)
